...
# interactions['session_id'] = interactions['session_id'].unique()
valid_actions = ['clickout item', 'interaction item rating', 'interaction item info',
        'interaction item image', 'interaction item deals','search for item']
action_dic = pd.Series(np.arange(len(valid_actions)), valid_actions)
interactions['action_type'] = action_dic[interactions['action_type']].values
interactions = interactions.dropna(subset=['action_type'])

interactions = interactions.rename(columns={'reference': 'item_id'})
interactions = interactions.rename(columns={'timestamp': 'created_at'})
# interactions = interactions.rename(columns={'action': 'action_type'})
...
## recsys2019
interactions.drop_duplicates(subset=['item_id', 'session_id', 'action_type'], keep='first', inplace=True)
inter_dense = interactions
...
# good_sessions = session_length[session_length >= 1].index
